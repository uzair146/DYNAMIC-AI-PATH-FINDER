# DYNAMIC AI PATH FINDER AGENT

A graphical pathfinding simulator that lets you watch **Greedy Best‑First Search** and **A* Search** in action on a grid. You can add obstacles, change the start and goal, and even turn on **dynamic mode** where new obstacles appear while the agent moves – forcing it to re‑plan in real time.
